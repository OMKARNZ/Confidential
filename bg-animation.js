...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Position Adjustment
    coinGroup.position.x = 4; // Moves coin to right side
    ...
    const animate = () => {
        ...
        coinGroup.rotation.z -= 0.002; // Constant spin
        coinGroup.rotation.y += 0.03 * (mouseX * 0.5 - coinGroup.rotation.y); // Mouse tilt
    ...
    window.addEventListener('resize', () => {
        ...
        if(window.innerWidth < 768) {
             ...
             coinGroup.position.z = -5; // Push back on mobile
        } else {
             coinGroup.position.x = 4;
             coinGroup.position.z = 0;
        }
    });
});
